...
// 闪传 sliceupload 的 Sha1StateV 需要每片对应的累积 SHA1 中间 state,
 ...
 * states[i] = 从初始 state 处理 file[0:(i+1)*sliceSize] 后的中间 state(小端 20B,不 finalize);
 * 最后一片 states[last] = 标准整文件 SHA1(有 finalize)。
 */
export function computeSha1StateV(
  bytes: Uint8Array,
  sliceCount: number,
  sliceSize: number,
): Uint8Array[] {
