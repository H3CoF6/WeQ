...
const FONT_IDS = [
  20125, 1, 20563, 20402, 20352, 22004, 20083, 20343, 20592, 20268, 20580, 20002, 21989, 22045,
  20717, 22043, 20010, 20486, 20131, 20112, 20730, 20748, 20161, 20704, 20188, 20185, 20716, 20359,
  20170, 20508, 20189, 20611, 20206, 20130, 20695, 20436, 20102, 20295, 20550, 20136, 20289, 20462,
  20018, 20506, 20271, 20107, 22020, 22018, 20377, 20183, 22005, 20003, 20012, 22040, 20504, 22041,
  22001, 20272, 20169, 22013, 22011, 20397, 20398, 20554, 20740, 20757, 20004, 22003, 20262, 20679,
  20724, 20171, 20247, 20162, 20266, 20705, 20050, 20182, 20752, 20735, 20191, 20294, 20001, 20551,
  20070, 20066, 20675, 20707, 20381, 22010, 20006, 22007, 20463, 20685, 20582, 20514, 20210, 20513,
  22002, 20467,
];
...
): Promise<FontUrlResult> {
  const result: FontUrlResult = { id, success: false };

  try {
    console.log(`[${id}] 获取下载链接...`);
    const nt = loadNative().ntHelper;
    const resource = await getFontResource(nt, info.pid, id, { from: 'WeQFontExport' });
...
async function main() {
  ...
  console.log('\n' + '='.repeat(60));
  console.log(
    `✅ 成功: ${success.length}/${FONT_IDS.length} (${((success.length / FONT_IDS.length) * 100).toFixed(1)}%)`,
  );
