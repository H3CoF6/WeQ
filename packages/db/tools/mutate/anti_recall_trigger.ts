 ...
 * gray-tip. Probing the live DB (see docs/guide/anti-recall.md) proved that
 * (40011=5, 40012=4) is a CLEAN, exclusive fingerprint of recall —
 ...
 *
 * Run:
 *   pnpm --filter @weq/db mutate:anti-recall-trigger status            # 只读，无需 --yes
 *   pnpm --filter @weq/db mutate:anti-recall-trigger install --yes     # backup + install
 *   pnpm --filter @weq/db mutate:anti-recall-trigger uninstall --yes   # remove triggers
 */

import { copyFileSync, existsSync, statSync } from 'node:fs';
import { loadNative } from '@weq/native';
import { QqDb } from '../../src/qq_db';
import { testEnv, requireMutationConsent } from '@weq/testkit';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    requireMutationConsent('在 nt_msg.db 上安装/卸载防撤回 SQL trigger');
    if (cmd === 'install') {
