...
import { testEnv, qqDbPath, requireMutationConsent } from '@weq/testkit';
...
async function main() {
  requireMutationConsent('读取指定 ARK 元素、改掉昵称后写回数据库');
  const nt = loadNative().ntHelper;
  const db = new QqDb(nt, { dbPath: DB_PATH, key: DB_KEY });

  const checkRows = await db.query(`SELECT rowid, "40001", "40800" FROM ${TABLE} WHERE rowid = ?`, [
    BigInt(ROWID),
  ]);
