      ...
      {rows.map((r) => (
        <div key={r.key} className="weq-usage-bar-row">
          <div className="weq-usage-bar-head">
            <span className="weq-usage-bar-label" title={r.label}>
              {r.label}
            </span>
...
function HourLine({
  ...
  data: Array<{ hour: string; tokens: number; calls: number }>;
}): ReactElement {
  const W = 560;
  const H = 140;
  const PAD_X = 8;
      ...
      <div className="weq-usage-line-body">
        ...
        <div className="weq-usage-line-plot">
          <svg
            ...
            className="weq-usage-line-svg"
            role="img"
            aria-label="近24小时token折线"
          >
            ...
            <line
              ...
              y1={PAD_Y}
              y2={H - PAD_Y}
              className="weq-usage-grid weq-usage-grid-axis"
            />
...
function RadarChart({ axes }: { axes: Array<{ label: string; value: number }> }): ReactElement {
  ...
  return (
    <svg viewBox="0 0 120 120" className="weq-usage-radar" role="img" aria-label="活跃时段雷达">
      ...
      {axes.map((axis, i) => {
        ...
        return (
          <line
            ...
            x2={p.x}
            y2={p.y}
            className="weq-usage-radar-grid"
          />
        );
      })}
      ...
      {axes.map((a, i) => {
        const p = at(i, maxR + 10);
        return (
          <text
            key={a.label}
            x={p.x}
            y={p.y}
            className="weq-usage-radar-label"
            textAnchor="middle"
    ...
    <ul className="weq-usage-legend">
      {rows.map((r) => (
        <li key={r.key}>
          ...
          <span className="weq-usage-legend-label" title={r.label}>
            {r.label}
          </span>
          <span className="weq-usage-legend-val">
...
export function UsagePanel({
  ...
  const modelSlices = useMemo(
    () =>
      withColors(
        (d?.byModel ?? []).map((m) => ({ key: m.model, label: m.model, value: m.tokens })),
      ),
    [d?.byModel],
  );
  ...
  if (!d || d.totalCalls === 0) {
    return (
      <div className="weq-agentlab-placeholder">
        ...
        <h3>用量统计</h3>
        <p>
          构建克隆或与克隆体聊天后，这里会展示 token 消耗、近 24 小时开销、场景 / 模型 / 时段 /
          克隆体的多维统计。
        </p>
      </div>
    );
    ...
    <div className="weq-usage">
      ...
      <section className="weq-usage-section">
        ...
        <div className="weq-usage-split">
          <span
            className="weq-usage-split-in"
            style={{ flexGrow: Math.max(0.001, d.promptTokens) }}
          />
          <span
            className="weq-usage-split-out"
            style={{ flexGrow: Math.max(0.001, d.completionTokens) }}
          />
        </div>
        <div className="weq-usage-split-legend">
          <span>
            <i className="weq-usage-dot is-in" /> 输入 {fmt(d.promptTokens)}（
            {Math.round((d.promptTokens / inOut) * 100)}%）
          </span>
          <span>
            <i className="weq-usage-dot is-out" /> 输出 {fmt(d.completionTokens)}（
            {Math.round((d.completionTokens / inOut) * 100)}%）
          </span>
        </div>
      </section>
      ...
      <div className="weq-usage-cols">
        <section className="weq-usage-section">
          ...
          <div className="weq-usage-chart">
            ...
            <div className="weq-usage-radar-hint">
              按一天 24 小时分 6 档（每 4 小时），看你在哪些时段最常用。
            </div>
          </div>
        </section>
        <section className="weq-usage-section">
