...
export function AddContactModal({
  ...
  const groupResult = useMemo(() => {
    ...
    return (
      conversations.find(
        (item) => item.type === 'group' && item.group.identityValue === identifier,
      ) ?? null
    );
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setSearched(true);
      setStatus(
        searchStatus(mode, Boolean(contactResult), Boolean(groupResult), contactResult?.kind),
      );
    }, 220);
  ...
  return (
    <div className={cn('modal-scrim')} role="presentation" onMouseDown={closeFromScrim(onClose)}>
      <section className={cn('app-modal add-contact-modal')} role="dialog" aria-modal="true">
        <header className={cn('modal-titlebar')}>
          <div>
            <h2>添加联系人</h2>
          </div>
          <button className={cn('icon-button')} type="button" onClick={onClose} title="关闭">
            <X size={22} />
          </button>
        </header>
          ...
          <section className={cn('add-contact-results')}>
            {mode === 'users' && contactResult ? (
              <div className={cn('search-user-row')}>
                ...
                <div>
                  ...
                  <span className={cn('copyable-text')}>ID {contactResult.user.identityValue}</span>
                </div>
                <button
                  className={cn('secondary-button')}
                  type="button"
                  disabled={contactResult.kind === 'self' || !('conversationId' in contactResult)}
                  onClick={() => {
            ...
            {!contactResult && !groupResult ? (
              <EmptyState
                title={
                  searched || status
                    ? status || (mode === 'groups' ? '没有找到这个群聊' : '没有找到这个 ID')
                    : mode === 'groups'
                      ? '按群号查找群聊'
                      : '按 ID 查找联系人'
                }
                body={
                  mode === 'groups' ? '输入群号，然后按 Enter。' : '输入用户 ID，然后按 Enter。'
                }
              />
            ) : null}
          </section>

          {status ? <p className={cn('form-status add-contact-status')}>{status}</p> : null}
        </main>
      </section>
    </div>
  );
...
export function CreateGroupModal({
  ...
  return (
    <div className={cn('modal-scrim')} role="presentation" onMouseDown={closeFromScrim(onClose)}>
      <section className={cn('app-modal create-group-modal')} role="dialog" aria-modal="true">
        <header className={cn('modal-titlebar')}>
          <div>
            <h2>创建群聊</h2>
          </div>
        ...
        <form className={cn('create-group-form')} onSubmit={submit}>
          ...
          <footer className={cn('modal-actions')}>
            <span>{selectedIds.size ? `已选择 ${selectedIds.size} 人` : status}</span>
            <button className={cn('secondary-button')} type="button" onClick={onClose}>
              取消
            </button>
          ...
          {status && selectedIds.size > 0 ? <p className={cn('form-status')}>{status}</p> : null}
        </form>
      </section>
    </div>
  );
