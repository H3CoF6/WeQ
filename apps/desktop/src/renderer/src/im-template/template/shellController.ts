...
export function useChatShellController({
  ...
  const [contactNotice, setContactNotice] = useState<ContactNoticeView | null>(null);
  const [selectedContactId, setSelectedContactId] = useState<string | null>(null);
  const [selectedGroupConversationId, setSelectedGroupConversationId] = useState<string | null>(
    null,
  );
  ...
  }, [activeConversationId, contactNotice, selectedContactId, selectedGroupConversationId, view]);
    ...
    () =>
      conversations.find(
        (conversation): conversation is GroupConversation =>
          conversation.type === 'group' && conversation.id === selectedGroupConversationId,
      ),
  ...
  const mainOpen = Boolean(
    activeConversation || selectedContact || selectedGroupConversation || contactNotice,
  );
  ...
  const switchView = useCallback(
    (nextView: MainView) => {
      ...
      setActiveConversationId(
        (current) =>
          current ??
          (history.shouldAutoSelectConversation() ? (conversations[0]?.id ?? null) : null),
      );
    },
