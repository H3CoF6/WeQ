...
export function EmojiPanel({
  ...
  const activePack = emojiPacks.find((pack) => pack.id === activePackId) ?? emojiPacks[0];
        ...
        {activePack.id === 'emoji' ? (
          <>
            <EmojiSection
              ...
              onSelect={onSelect}
            />
            <EmojiSection title="表情" items={activePack.items} onSelect={onSelect} />
          </>
        ) : activePack.type === 'text' ? (
          <KaomojiSection pack={activePack} onSelect={onSelect} />
        ) : (
          <EmojiSection title={activePack.name} items={activePack.items} onSelect={onSelect} />
        )}
      </div>
...
function EmojiTabPreview({ pack }: { pack: EmojiPack }) {
  ...
  return preview ? <img src={preview.value} alt={pack.name} draggable={false} /> : null;
}
