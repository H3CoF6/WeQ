...
import { defaultToolRegistry, type ToolPaneItem, type ToolPaneGroup } from './toolRegistry';
      ...
      <div className={cn('tools-list')}>
        {groups.map((group) => (
          <div className={cn('tools-group')} key={group.id}>
            {group.label ? <h3 className={cn('tools-group-title')}>{group.label}</h3> : null}
            <div className={cn('tools-group-items')}>
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <span className={cn('tools-row-icon')} style={{ color: item.color }}>
                      <Icon size={28} strokeWidth={2.4} />
                    </span>
                    <span className={cn('tools-row-main')}>
                      <span className={cn('tools-row-label')}>{item.label}</span>
                      {item.description ? (
                        <span className={cn('tools-row-description')}>{item.description}</span>
                      ) : null}
                    </span>
...
export function ToolDetailPane({
  ...
  const activeEntry = entries.find((entry) => entry.item.id === selectedItemId) ?? entries[0];
  ...
  const relatedEntries = entries.filter((entry) => entry.group.id === activeEntry.group.id);

  return (
    <section className={cn('tools-detail-pane')}>
      <div className={cn('tools-detail-inner')}>
        <header className={cn('tools-detail-head')}>
          <span className={cn('tools-detail-icon')} style={{ color: activeItem.color }}>
            <ActiveIcon size={48} strokeWidth={2.2} />
          </span>
          <div>
            <span className={cn('tools-detail-kicker')}>{activeEntry.group.label ?? '应用'}</span>
