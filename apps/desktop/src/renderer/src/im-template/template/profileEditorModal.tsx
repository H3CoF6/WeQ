...
export function ProfileEditorModal({
  ...
  return (
    <div className={cn('modal-scrim')} role="presentation" onMouseDown={closeFromScrim(onClose)}>
      <section className={cn('app-modal profile-editor-modal')} role="dialog" aria-modal="true">
        ...
        <ProfileEditorForm user={user} onClose={onClose} onUserChange={onUserChange} />
      </section>
    </div>
  );
}
...
export function ProfileEditorForm({
  ...
  const [avatarSource, setAvatarSource] = useState<AvatarSource>(initialAvatar.source);
  ...
  const previewAvatarUrl = avatarInput.trim() ? resolvedAvatarUrl : user.avatarUrl;
  ...
  return (
    <form className={cn('profile-editor-form')} onSubmit={saveAvatar}>
      ...
      <div className={cn('profile-editor-source')}>
        <span>头像</span>
        <div className={cn('avatar-source-tabs')} role="tablist" aria-label="头像来源">
          <button
            type="button"
      ...
      {status ? <p className={cn('form-status profile-editor-status')}>{status}</p> : null}
      <div className={cn('profile-editor-actions')}>
        <button type="button" className={cn('secondary-button')} onClick={onClose}>
          取消
        </button>
