// @ts-nocheck
import { Bell, BookOpen, Github, Info, MessageCircle, Settings, UserPlus, X } from 'lucide-react';
...
export function AboutModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className={cn('modal-scrim')} role="presentation" onMouseDown={closeFromScrim(onClose)}>
      <section className={cn('app-modal compact-modal')} role="dialog" aria-modal="true">
    ...
    <div className={cn('modal-scrim')} role="presentation" onMouseDown={closeFromScrim(onClose)}>
      ...
      >
        ...
        <main className={cn('modal-main')}>
          ...
          {active === 'intro' ? <IntroHelp /> : <HelpArticle page={helpPages[active]} />}
        </main>
      </section>
    </div>
  );
...
type HelpTab = 'intro' | 'chat' | 'notifications' | 'contacts' | 'interface' | 'docs';
    ...
    <>
      <section className={cn('modal-card help-intro-card')}>
        <div className={cn('help-intro-brand')}>
          ...
          <div>
            <h3>Webark IM Template</h3>
            <p>面向 IM 产品的前端模板，包含聊天、联系人、群资料、设置和帮助页面。</p>
          </div>
        </div>
