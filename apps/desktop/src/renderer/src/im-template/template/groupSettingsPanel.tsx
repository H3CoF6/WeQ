...
import type { GroupConversationView, GroupUpdateInput } from './conversationDetailsTypes';
...
export function GroupSettingsPanel({
  ...
  onUpdateGroup: (conversationId: string, input: GroupUpdateInput) => Promise<void>;
  ...
  const [announcement, setAnnouncement] = useState(conversation.group.announcement ?? '');
  const [avatarSource, setAvatarSource] = useState<'github' | 'weavatar'>(initialAvatar.source);
  ...
  async function saveAvatar() {
    ...
    if (!ref) {
      setStatus(avatarSource === 'github' ? '请输入 GitHub 用户名' : '请输入 32 位 MD5');
      return;
    }
  ...
  return (
    ...
    >
      ...
      {canManage && !profileEditing ? (
        <section className={cn('details-card group-edit-card group-edit-preview')}>
      ...
      {canManage ? (
        <section className={cn('details-card group-avatar-settings')}>
          ...
          <input
            ...
            placeholder={avatarSource === 'github' ? '输入 GitHub 用户名' : '输入 32 位 MD5'}
          />
      ...
      {status ? <span className={cn('form-status compact')}>{status}</span> : null}

      <DesktopDetailActionGroups groups={detailActionGroups} context={detailActionContext} />
    </aside>
  );
}
...
function groupAvatarInputFromUrl(avatarUrl: string | null | undefined): {
  ...
  const github = avatarUrl.match(/^https:\/\/github\.com\/([a-zA-Z0-9-]+)\.png\?size=240$/);
  if (github?.[1]) {
    return {
      source: 'github',
      ref: github[1],
  ...
  const weavatarMatch = avatarUrl.match(/^https:\/\/weavatar\.com\/avatar\/([a-fA-F0-9]{32})/);
  if (weavatarMatch) {
    return {
      source: 'weavatar',
