// @ts-nocheck
import { ChevronLeft, ChevronRight, Moon, QrCode, Settings, X } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <footer className={cn('mobile-profile-footer')}>
        ...
        />
        <ProfileFooterButton icon={<Moon size={33} />} label="夜间" onClick={toggleTheme} />
      </footer>
    </section>
  );
}
      ...
      <main className={cn('mobile-profile-qr-main')}>
        <section className={cn('mobile-profile-qr-card')}>
          ...
          <div className={cn('mobile-profile-qr-code')}>
            {qrDataUrl ? <img src={qrDataUrl} alt="我的二维码" /> : <span>生成中</span>}
            {qrDataUrl ? (
...
function ProfileFooterButton({
  ...
  return (
    <button className={cn('mobile-profile-footer-button')} type="button" onClick={onClick}>
      {icon}
      <span>{label}</span>
    </button>
  );
