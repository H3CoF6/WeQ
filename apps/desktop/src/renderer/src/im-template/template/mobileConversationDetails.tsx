...
export function MobileConversationDetails({
  ...
  const actionRegistry = resolveConversationDetailActionRegistry(conversationDetailActions);
...
function MobileHeader({
  ...
  return (
    <header className={cn('mobile-detail-header')}>
      <button className={cn('mobile-detail-back')} type="button" title="返回" onClick={onClose}>
...
function MobileDirectDetails({
  ...
  if (profileOpen) {
    return <MobileDirectProfilePage contact={other} onBack={() => setProfileOpen(false)} />;
  }
  ...
  return (
    <>
      ...
      <main className={cn('mobile-detail-main')}>
        ...
        <MobileDetailActionGroups groups={actionGroups} context={actionContext} />
      </main>
    </>
  );
}
      ...
      <main className={cn('mobile-detail-main')}>
        <section className={cn('mobile-group-summary')}>
          ...
          <div className={cn('mobile-group-copy')}>
            ...
            <span>
              {conversation.group.identityLabel}: {conversation.group.identityValue}
            </span>
          </div>
          <button
          ...
          <div className={cn('mobile-member-grid')}>
            {members.slice(0, 15).map((member) => (
              <div className={cn('mobile-member-item')} key={member.id}>
                ...
                />
                <span className={cn('mobile-member-name')}>{displayUserName(member)}</span>
              </div>
            ))}
            {members.length === 0 ? (
...
function MobileDirectProfilePage({
  ...
  return (
    <>
      ...
      <main className={cn('mobile-detail-main')}>
        <section className={cn('mobile-group-summary', 'mobile-direct-profile-summary')}>
          <Avatar
        ...
        <MobileCard>
          ...
          <MobileInfoRow label={contact.identityLabel} value={contact.identityValue} />
      ...
      <main className={cn('mobile-profile-qr-main')}>
        <section className={cn('mobile-profile-qr-card')}>
          ...
          <div className={cn('mobile-profile-qr-code')}>
            {qrDataUrl ? <img src={qrDataUrl} alt="群二维码" /> : <span>生成中</span>}
            {qrDataUrl ? (
...
function groupJoinQrUrl(conversation: Extract<Conversation, { type: 'group' }>) {
  const origin =
    typeof window === 'undefined' ? 'https://im-template.local' : window.location.origin;
...
function MobileCard({ children, className }: { children: ReactNode; className?: string }) {
  return <section className={cn('mobile-detail-card', className)}>{children}</section>;
}
      ...
      {groups.map((group) => (
        <MobileCard key={group.id}>
          {group.actions.map((action) => {
            ...
            const disabled = isConversationDetailActionDisabled(action, context);
...
function MobileInfoRow({
  ...
  return (
    <button
      ...
      onClick={onClick}
    >
      {icon ? <span className={cn('mobile-detail-row-icon')}>{icon}</span> : null}
      <span className={cn('mobile-detail-row-label', labelClassName)}>{label}</span>
      <span
        className={cn('mobile-detail-row-value', multiline && 'mobile-detail-row-value-multiline')}
      >
        {value}
      </span>
...
function MobileSwitchRow({
  ...
  return (
    <button
      className={cn('mobile-detail-switch-row', !icon && 'mobile-detail-switch-row-no-icon')}
      type="button"
      role="switch"
      ...
      onClick={onClick}
    >
      {icon ? <span className={cn('mobile-detail-switch-icon')}>{icon}</span> : null}
