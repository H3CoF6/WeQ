...
import { resolveSettingsPanelRegistry, type SettingsPanelRegistry } from './settingsPanels';
...
export function SettingsModal({
  ...
  const activeExtensionPanel = settingsPanelRegistry.desktop.find((panel) => panel.id === active);
  ...
  return (
    <div className={cn('modal-scrim')} role="presentation" onMouseDown={closeFromScrim(onClose)}>
      <section className={cn('app-modal settings-modal')} role="dialog" aria-modal="true">
        ...
        <main className={cn('modal-main')}>
          <header className={cn('modal-titlebar')}>
            <div>
              <h2>
                {modalTitle(active, activeExtensionPanel?.title ?? activeExtensionPanel?.label)}
              </h2>
            </div>
          ...
          {active === 'general' ? (
            <>
              <section className={cn('modal-card appearance-card')}>
                <h3>外观设置</h3>
                <ThemeSelector value={themePreference} onChange={onThemePreferenceChange} />
              </section>
          ...
          {activeExtensionPanel ? activeExtensionPanel.render(settingsPanelContext) : null}
        </main>
      </section>
      <MobileSettingsPage
        user={user}
...
function MobileSettingsPage({
  ...
  return (
    <section className={cn('mobile-settings-page')} role="dialog" aria-modal="true">
      <header className={cn('mobile-settings-header')}>
        <button className={cn('mobile-settings-back')} type="button" title="返回" onClick={onClose}>
      ...
      <main className={cn('mobile-settings-main')}>
        ...
        <MobileSettingsSection title="功能">
          <MobileSettingsCard>
            ...
            />
            <MobileSettingsRow icon={<SlidersHorizontal size={28} />} label="通用" />
          </MobileSettingsCard>
              ...
              {section.rows.map((row) => {
                ...
                return (
                  <MobileSettingsRow
                    ...
                    value={row.value}
                    onClick={
                      row.onClick ? () => void row.onClick?.(settingsPanelContext) : undefined
                    }
                  />
                );
              })}
        ...
        <MobileSettingsSection title="隐私">
          <MobileSettingsCard>
            <MobileSettingsRow icon={<LockKeyhole size={28} />} label="隐私设置" />
          </MobileSettingsCard>
        </MobileSettingsSection>
      </main>
    </section>
...
function MobileSettingsSection({ title, children }: { title: string; children: ReactNode }) {
  return (
...
function MobileSettingsRow({
  ...
  return (
    <button className={cn('mobile-settings-row')} type="button" onClick={onClick}>
      <span className={cn('mobile-settings-row-icon')}>{icon}</span>
      <span className={cn('mobile-settings-row-label')}>{label}</span>
      <span className={cn('mobile-settings-row-value')}>{trailing ?? value}</span>
    ...
    <div className={cn('theme-grid')} role="radiogroup" aria-label="外观模式">
      {options.map((option) => (
        <button
          key={option.value}
          type="button"
          className={cn(`theme-option ${value === option.value ? 'active' : ''}`)}
          role="radio"
          aria-checked={value === option.value}
          onClick={() => onChange(option.value)}
        >
          <span className={cn(`theme-preview ${option.value}`)} aria-hidden="true">
...
function NotificationBadge({ configured, mode }: { configured: boolean; mode: NotificationMode }) {
  if (mode === 'off') {
...
function ModeButton({
  ...
  return (
    <button type="button" className={cn(`mode-button ${active ? 'active' : ''}`)} onClick={onClick}>
      {icon}
      {label}
    </button>
  );
