...
export function loadHiddenMessageIds(conversationId: string | undefined) {
  ...
  try {
    const raw = window.localStorage.getItem(hiddenMessageStorageKey(conversationId));
...
export function saveHiddenMessageIds(conversationId: string, ids: Set<string>) {
  try {
    window.localStorage.setItem(hiddenMessageStorageKey(conversationId), JSON.stringify([...ids]));
  } catch {
