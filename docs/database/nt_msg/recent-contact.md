...
> 它会把「有草稿」的会话按草稿时间提前。
>
> 上面的 SQL 是 WeQ 取**会话行**的读法（按 `40050` 分页拉窗口）；真正决定列表次序的排序
> 在渲染层重做：置顶会话按置顶时间（`41103`）排最前，其余按
> **`max(40050, 41108)`** 倒序 —— 即「最后消息时间」与「草稿时间」取较新者，让打了字
> 的会话冒头。为此额外读一条轻量查询：
>
> ```sql
...
> ```
>
> 与 QQ 的 `41136` 有一处差别：`41136` 的官方语义到底是「有草稿就取草稿时间」还是
> 「取较新者」尚未验证（见上文「未验证的两点」），WeQ 选的是后者 —— 两者的差别只会在
> 「仓库里有旧草稿、之后又来了新消息」时显现。另外 WeQ 排序**只认库里的时间**：输入框里
> 正在编辑、还没写回库的内容不参与排序（写回时机见
> [draft-storage · 写入策略](./draft-storage.md)）。
