...
| tag | 字段名 | 类型 | 含义 |
| --- | ------ | ---- | ---- |
| 45004 | `localPath` | string | 本地缓存路径。**这个 tag 落在通用段而非 476xx 段，也不是 FACE 专属** —— 在 PIC 上实测是原图的本机缓存路径（原名 `faceExtDesc`（扩展描述）系误判，见 [pic.md](./pic.md)）。FACE 上一直没见到非空样本，故「扩展描述」降级为存疑 |
